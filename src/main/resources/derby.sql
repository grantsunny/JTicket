...
DROP VIEW TKT.TicketingSeatStatus;
DROP VIEW TKT.SessionStatistics;
DROP VIEW TKT.EventStatistics;
DROP VIEW TKT.SKU;
DROP VIEW TKT.SeatsInEvent;
DROP VIEW TKT.SeatDetails;
DROP TRIGGER TKT.PreventPaidOrderRemoval;
DROP TRIGGER TKT.PreventEventTimeOverLap;
DROP PROCEDURE TKT.RaiseException;
...
SELECT
    ...
    END AS available,
    CASE
        WHEN Seats.AVAILABLE = FALSE THEN CAST('{"ticketingAvailabilityReason":"physicalUnavailable"}' AS CLOB)
        WHEN T.priceId IS NULL THEN CAST('{"ticketingAvailabilityReason":"unpriced"}' AS CLOB)
        ELSE Seats.METADATA
    END AS metadata,
    PRICES.PRICE,
